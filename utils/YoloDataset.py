...
class YoloDataset(data.Dataset):
    def __init__(self,
        ...
        self.class_counts = self.count_class_distribution()
        total = self.class_counts.sum()
        for i, c in enumerate(self.class_counts):
            print(f"class {i}: {c} ({c/total:.2%})")
        ...
        if label_path not in self.label_cache:
            ...
            with open(label_path, 'r') as f:
                for line in f.read().strip().splitlines():
                    ...
                    if int(cls) >= self.cls_num:
                        raise ValueError(f"class id {cls} >= cls_num {self.cls_num} from {label_path}")
    ...
    def count_class_distribution(self):
        class_counts = np.zeros(self.cls_num, dtype=int)
...
def show_images_with_boxes(train_dataset, mean, std):
    ...
    for img_tensor, boxes, labels, img_id in train_dataset:
        # -------------------------
        # Tensor -> numpy
        # -------------------------
        ...
        # ⚠️ matplotlib用RGB，不要转BGR
        # img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR) ❌ 删掉
        ...
        # 控制节奏（代替 waitKey）
        # -------------------------
        user_input = input("回车下一张，输入 q 退出: ")
...
if __name__ == '__main__':
    ...
    data_name = "F"
    data_root = r"/root/autodl-tmp/" + data_name
